    ...
    var TableContentsView = SingleView.extend({
      ...
      _itemRequested: function(event, itemNum) {
        ...
        if (settings.table) {
          ...
          //text = (pages.length==1) ? 'Sayfa '+pages[0] : 'Sayfalar '+pages.join('-');
		  text = (pages.length==1) ? 'Sayfa '+pages[0] : 'Sayfalar '+ (pages[0]-1) +"-"+ (pages[1]-1);
        if(itemNum==0){
			 text= 'Kapak';	
		  }
      if ($('#flipbook').turn('display') != 'single') {
    ...
    var ShareBox = SingleView.extend({
      ...
      initialize: function() {
        ...
        html +='<div class="ui-share-options">';
          html +='<a href="https://www.facebook.com/MTTBilisimLisesi/" target="_blank" title="Facebook" class="ui-icon show-hint"><i class="bi bi-facebook"></i></a>';
          html +='<a href="https://twitter.com/bilisimlisesi" target="_blank" title="Twitter" class="ui-icon show-hint"><i class="bi bi-twitter"></i></a>';
          html +='<a href="https://www.instagram.com/bilisimprojelisesi/" target="_blank" title="Instagram" class="ui-icon show-hint"><i class="bi bi-instagram"></i></a>';
          html +='<a href="https://www.linkedin.com/company/mehmet-tu%C4%9Frul-tekbulut-mesleki-ve-teknik-anadolu-lisesi/" target="_blank" title="Linkedin" class="ui-icon show-hint"><i class="bi bi-linkedin"></i></a>';
        html +='</div>';
